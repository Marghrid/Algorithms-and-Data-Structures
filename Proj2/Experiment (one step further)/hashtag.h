...
// increase_hashtag(h_tag): incrementa o numero de ocorrencias da hashtag
//                          cuja morada e h_tag;
...
// compare_hashtag_n_string(h_tag, texto): compara uma hashtag passada por referencia (h_tag) e uma string (texto).
//             A hashtag diz-se menor (resp. igual/maior) do que a string 
//             se o seu texto for lexicograficamente menor (resp. igual/maior)
//             do que o texto na string;
...
int compare_hashtag(void*, void*);

// compare_hashtag_pointer(h_tag1, h_tag2): compara duas hashtags cuja morada e passada por 
//                                          referencia, com base na ordem pedida no comando l;
int compare_hashtag_pointer(const void*,const void*);

// hash_string(texto, size):  funcao de dispersao de uma hashtag, que recebe uma string texto
//                            e um inteiro 'size' com o tamanho da hashtable;
int hash_string(void*, int);

// hash_hashtag(h_tag, size): funcao de dispersao de uma hashtag, que recebe um ponteiro
//                            para uma hashtag (h_tag) e um inteiro com o tamanho da hashtable (size);
